const { rotate } = require("../src/189-rotate-array_o-n");
...
test("given an array, should rotate the array to the right by k steps (Example 4)", () => {
  let nums = [-1];
  const k = 2;
  rotate(nums, k);
...
test("given an array, should rotate the array to the right by k steps (Example 5)", () => {
  let nums = [1, 2];
  const k = 3;
  rotate(nums, k);
